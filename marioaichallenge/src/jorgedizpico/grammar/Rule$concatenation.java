...
final public class Rule$concatenation extends Rule
{
  private Rule$concatenation(String spelling, ArrayList<Rule> rules)
  {
  ...
  public static Rule$concatenation parse(ParserContext context)
  {
    context.push("concatenation");
            ...
            if (!parsed)
            {
              /* concatenation - start */
              {
                ...
                if (parsed)
                {
                  ...
                  for (int i2 = 0; i2 < 1 && f2; i2++)
                  {
                    rule = Rule$weight.parse(context);
                    if ((f2 = rule != null))
                    {
                      e2.add(rule);
                      c2++;
    ...
    rule = null;
    if (parsed)
      rule = new Rule$concatenation(context.text.substring(s0, context.index), e0);
    else
      context.index = s0;

    context.pop("concatenation", parsed);

    return (Rule$concatenation)rule;
  }
}
