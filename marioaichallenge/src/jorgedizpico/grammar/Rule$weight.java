...
final public class Rule$weight extends Rule
{
  private Rule$weight(String spelling, ArrayList<Rule> rules)
  {
  ...
  public static Rule$weight parse(ParserContext context)
  {
    context.push("weight");
    ...
    rule = null;
    if (parsed)
      rule = new Rule$weight(context.text.substring(s0, context.index), e0);
    else
      context.index = s0;

    context.pop("weight", parsed);

    return (Rule$weight)rule;
  }
}
